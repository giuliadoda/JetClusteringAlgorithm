#!/bin/bash
#
# Runs the OpenMP binary with different OMP_SCHEDULE settings (and optionally
# different thread counts) and collects the total execution time into a CSV.
#
# Usage:
...
RUNS="${2:-1}"                     # how many repetitions per configuration (for averaging)
...
for sched_entry in "${SCHEDULES[@]}"; do
    ...
    for threads in "${THREAD_COUNTS[@]}"; do
        for run in $(seq 1 "$RUNS"); do
            ...
            echo "${sched_name},${sched_chunk:-default},${threads},${run},${time_sec}" >> "$OUTPUT_CSV"
